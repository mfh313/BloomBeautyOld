...
-(id) initWithFMDB:(FMDatabase *)database{
  self = [super init];
  if(self){
      self.primaryId = @"0";//NSNotFound;
      self.primaryField = @"ID";
      self.isAutoIncrement = NO;
//    [self getNowDatabase] = database;
  }
  return self;
}
...
//根据主键删除
-(BOOL) deleteWithPrimary:(NSString*)primaryId
{
  NSString *condition = [self getPrimaryIdCondition:primaryId];//[NSString stringWithFormat: @" AND %@ = %@", self.primaryField, primaryId];
...
-(BOOL) deleteWithCondition:(NSString *)cond parameters:(NSArray *)params
{
  NSString *sql = [NSString stringWithFormat: @"DELETE FROM %@ WHERE 1 = 1%@",
                   self.tableName,
                   [PeakSqlite getString: cond]];
  [[self getNowDatabase] open];
  BOOL result = [[self getNowDatabase] executeUpdate:sql withArgumentsInArray:params];
  [[self getNowDatabase] close];
  return result;
}
...
-(NSInteger) insertWithSql:(NSString *)sql parameters:(NSArray *)params
{
  NSInteger lastId = -1;
  [[self getNowDatabase] open];
  if([[self getNowDatabase] executeUpdate:sql withArgumentsInArray:params]){
    lastId = [[self getNowDatabase] lastInsertRowId];
  }
  [[self getNowDatabase] close];
  return lastId;
}
...
-(id) scalarWithSql:(NSString *)sql parameters:(NSArray *)params
{
  id result = nil;
  [[self getNowDatabase] open];
  FMResultSet *rs = [[self getNowDatabase] executeQuery:sql withArgumentsInArray:params];
...
-(BOOL) findOneWithCondition:(NSString *)cond parameters:(NSArray *)params orderBy:(NSString *)orderBy
{
  ...
  [[self getNowDatabase] open];
   NSArray *arry = [self findWithSql:sql parameters:params];
...
//查询 根据id 条件
-(NSString*)getPrimaryIdCondition:(NSString*) primaryId{
    NSString *cond = @"";
    if([self.primaryIdType isEqualToString:@"integer"])
    {
        cond = [NSString stringWithFormat: @" AND %@ = %@", self.primaryField, primaryId];
    }else
    {
        cond = [NSString stringWithFormat: @" AND %@ = '%@'", self.primaryField, primaryId];
    }
...
//根据主键，获取一条记录
-(BOOL)findOneWithPrimaryId:(NSString*) primaryId{
...
//        cond = [NSString stringWithFormat: @" AND %@ = '%@'", self.primaryField, primaryId];
//    }
    NSString *cond = [self getPrimaryIdCondition:primaryId];
  return [self findOneWithCondition:cond parameters:nil orderBy:nil];
}


-(id)findOneObjWithCondition:(NSString *)cond parameters:(NSArray *)params orderBy:(NSString *)orderBy
{
    BOOL bol = [self findOneWithCondition:cond parameters:params orderBy:orderBy];
    if(bol)
    {
        return self.data ;
    }else
...
//根据主键，获取一条记录
-(id)findOneObjWithPrimaryId:(NSString*) primaryId{
    NSString *cond = [self getPrimaryIdCondition:primaryId];
    return [self findOneObjWithCondition:cond parameters:nil orderBy:nil];
}
...
-(FMDatabase *) getNowDatabase{
   return  [MFDatabaseUtil sharedManager].database;
}
