...
/** Represents the results of executing a query on an `<MFDatabase>`.
...
@interface MFResultSet : NSObject {
    MFDatabase          *_parentDB;
    MFStatement         *_statement;
...
/** `MFStatement` used by result set. */

@property (atomic, retain) MFStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<MFStatement>`
 
 @param statement A `<MFStatement>` to be performed
 
 @param aDB A `<MFDatabase>` to be used
 
 @return A `MFResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(MFStatement *)statement usingParentDatabase:(MFDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(MFDatabase *)newDb;
