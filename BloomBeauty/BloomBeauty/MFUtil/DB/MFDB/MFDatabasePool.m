...
@interface MFDatabasePool()

- (void)pushDatabaseBackInPool:(MFDatabase*)db;
- (MFDatabase*)db;

@end


@implementation MFDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return MFDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"MFdb.%@", self] UTF8String], NULL);
        _databaseInPool     = MFDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = MFDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    MFDBRelease(_path);
    MFDBRelease(_databaseInPool);
    MFDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        MFDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(MFDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The MFDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (MFDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [MFDatabase databaseWithPath:self->_path];
