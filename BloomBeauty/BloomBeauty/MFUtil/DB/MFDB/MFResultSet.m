...
+ (instancetype)resultSetWithStatement:(MFStatement *)statement usingParentDatabase:(MFDatabase*)aDB {
    ...
    return MFDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    MFDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return MFDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"MFDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
